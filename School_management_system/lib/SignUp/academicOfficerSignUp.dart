...
class _AcademicOfficerSignupPageState extends State<AcademicOfficerSignupPage> {
  ...
  Widget _buildFormLayout(BuildContext context) {
    ...
    if (AppThemeResponsiveness.isMobile(context)) {
      ...
      columns = 2; // For desktop, we can have 3 columns for academic officer form
    }
  ...
  void _handleAcademicOfficerSignup() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final parsedDate = _parseDateFromString(_dateOfJoiningController.text);
        ...
        // Call actual API
        final response = await ApiService().registerAcademicOfficer(
        ...
        if (mounted) {
          if (response.success) {
            AppSnackBar.show(
              context,
              message: response.message ?? 'Academic Officer account created successfully!',
